...
object LunaSoftMessageMapper {
    ...
    fun getNotificationTalkTemplateId(state: String): Int {
        return when (state) {
            zupzup.manager.data.common.Constants.stateConfirm -> zupzup.manager.data.common.Constants.confirmTemplateId
            zupzup.manager.data.common.Constants.stateCancel -> zupzup.manager.data.common.Constants.rejectTemplateId
            zupzup.manager.data.common.Constants.statePartialConfirm -> zupzup.manager.data.common.Constants.partialConfirmTemplateId
            else -> 0
        }
    }
    ...
    fun getNotificationTalkMessage(
        ...
        val messageContent = when (state) {
            zupzup.manager.data.common.Constants.stateConfirm -> getConfirmMessageContent(orderModel)
            zupzup.manager.data.common.Constants.statePartialConfirm -> getPartialConfirmMessageContent(orderModel)
            zupzup.manager.data.common.Constants.stateCancel -> getCancelMessageContent(orderModel)
            else -> ""
        }
        list.add(
            Message(
    ...
    private fun getConfirmMessageContent(orderModel: OrderModel): String {
        return "<예약 확정 안내>\n" +
                ...
                "-예약 매장: ${zupzup.manager.data.common.Constants.storeName}\n" +
